...
def insert_view(request):
    global pid, age, gender, bmi, bsa, ecog, atta, diabetes, hdci, ef, wl, hv, wbc, anc, pc, src, cc, bili, astalt, albu, spo, hr, pho, bp, lw , apcw, coca, di, sett, ss, dfh, ms, ct, gra, sb, pct, psdc,disthome
    form = insert()
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            p = cancerdb(
                m_Patient_Id=pid,m_Age=age, m_Gender=gender, m_BMI=bmi, m_BSA=bsa, m_ECOG=ecog, m_Substanceabuse=atta,
            ...
            p.save()
            l2 = [pid, age, gender, bmi, bsa, ecog, atta, diabetes, hdci, ef, wl, hv, wbc, anc, pc, src, cc, bili, astalt, albu, spo, hr, pho, bp, lw, apcw, coca, di, sett, ss,disthome , dfh, ms, ct, gra, sb, pct, psdc]
            ans = calculation_view()
            return render(request,'calculate.html',{'post':l2,'l':ans})
    return render(request, 'insert.html', {'form': form})
# def update1_view(request):
#     global pid, age, gender, bmi, bsa, ecog, atta, diabetes, hdci, ef, wl, hv, wbc, anc, pc, src, cc, bili, astalt, albu, spo, hr, pho, bp, lw, apcw, coca, di, sett, ss, dfh, ms, ct, gra, sb, pct, psdc
#     form = update1()
#     if request.method == 'POST':
...
#     patient=cancerdb.objects.get(m_Patient_Id=id)
...
def search_view(request):
    ...
    if(request.method == 'POST'):
        ...
        if(form.is_valid()):
            pid=form.cleaned_data['Patient_Id']
            p=cancerdb.objects.get(m_Patient_Id=pid)
            return render(request,'search.html',{'form':form,'key':p })
...
def calculation_view():
    ...
    if bsa >= 1 and bsa <= 1.1:
        val_bsa = 0 
    elif bsa == 1.2:
        val_bsa = 1
    elif bsa >= 1.3 and bsa <= 1.7:
        val_bsa = 2
    elif bsa == 1.8:
        val_bsa = 3
    elif bsa == 1.9:
    ...
    if 0 <= risk_score < 0.1:
        l1.append('5-Fluorouracil (5-FU)')
    elif 0.1 <= risk_score < 0.2:
        l1.append('Capecitabine (Xeloda)')
